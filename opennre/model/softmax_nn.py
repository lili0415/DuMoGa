...
class SoftmaxNN(SentenceRE):
    ...
    def __init__(self, sentence_encoder, num_class, rel2id):
        ...
        self.fc = nn.Linear(self.sentence_encoder.hidden_size, self.sentence_encoder.hidden_size)
    ...
    def forward(self,tokens, att_mask, list_p, list_r):
        """
        Args:
        ...
        Return:
            logits, (B, N)
        """
        rep = self.sentence_encoder(tokens, att_mask, list_p, list_r)  # (B, H)
        #rep = self.drop(rep)
        logits = self.fc(rep)  # (B, N)
        logits = self.fc2(logits)
        #logits = nn.functional.gumbel_softmax(logits,dim=1)
        return logits
