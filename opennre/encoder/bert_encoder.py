...
class BERTEntityEncoder(nn.Module):
    def __init__(self, max_length, pretrain_path, blank_padding=True, mask_entity=False):
        ...
        self.pic_feat = 1024   #original 4096
        self.mask_entity = mask_entity
        logging.info('Loading BERT pre-trained checkpoint.')
        self.bert = BertModel.from_pretrained(pretrain_path)
        self.tokenizer = BertTokenizer.from_pretrained(pretrain_path)
    ...
    def forward(self, token, att_mask, pic, rel):
        """
        Args:
            token: (B, L), index of tokens
            att_mask: (B, L), attention mask (1 for contents and 0 for padding)
        ...
        # structural alignment and the combination of semantic graph alignment  (FULL)
        ...
        #pic_out = torch.sum(pic_rel, dim=1)

        # # structural alignment and the combination of semantic graph alignment  (GRAPH)
        # rel = rel.view(-1, 10, self.max_length)
        # pic_rel = torch.matmul(rel, hidden_rel)
        # # pic_out = torch.cat([pic, pic_rel], dim=-1)
        # # pic_out = self.linear_final1(pic_out)
        ...
        # structural alignment and the combination of semantic graph alignment  (FEATURE)
        # rel = rel.view(-1, 10, self.max_length)
        # pic_rel = torch.matmul(rel, hidden_rel)
        #pic_out = torch.cat([pic, pic_rel], dim=-1)
        #pic_out = self.linear_final1(pic_out)
       # pic_out = torch.sum(pic, dim=1)
        #pic_out = torch.sum(pic_rel, dim=1)
    ...
    def tokenize(self,item):
        sent = self.tokenizer.tokenize(item)
        re_tokens = ['[CLS]'] + sent + ['[SEP]']
        indexed_tokens = self.tokenizer.convert_tokens_to_ids(re_tokens)
        avai_len = len(indexed_tokens)
